...
class Describer implements CheckedFunction1<Git, Describer.Described> {
   ...
   */
  Describer(final ObjectId objectId) {
    this.oid = objectId;
  }
   ...
   */
  static CheckedFunction1<Git, Describer.Described> describe(final ObjectId objectId) {
    return new Describer(objectId);
  }

  @Override
  public @Nullable Described apply(final Git git) throws InvalidPatternException, IOException, GitAPIException {
  ...
  static class Described {
     ...
     */
    Described(final List<String> partsList) {
